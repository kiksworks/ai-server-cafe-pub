...
public class ActionProtectBall extends AbstractAction {
    ...
    public ActionProtectBall(int id, TeamColor color) {
        super(id, color);
    ...
    protected Command execute() {
        ...
        // 状態遷移
        switch (this.state) {
            case ActionProtectBall.RunningState.MOVE:
                if (haveBall) {
                    this.state = ActionProtectBall.RunningState.ROUND;
                }
                break;

            case ActionProtectBall.RunningState.ROUND:
                if (distBtoR > ROBOT_RAD * 2 || MathHelper.inferiorAngle(toBallTheta, robot.getTheta()) > 0.8 * FastMath.PI) {
                    this.state = ActionProtectBall.RunningState.MOVE;
                }
                break;
        }
            ...
            case RunningState.ROUND:
                ...
                if (oppRobot.isPresent()) {
                    ...
                    if (this.haveBall && MathHelper.distance2D(oppPos, ballPos) < 1000) {
                        ...
                        // ボールを持っているとき
                        // 敵から離れる方向
                        toTargetTheta = MathHelper.direction(ballPos, oppPos);
