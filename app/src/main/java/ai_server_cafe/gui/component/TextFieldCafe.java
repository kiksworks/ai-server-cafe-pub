...
public class TextFieldCafe extends JTextField implements IComponentCafe {
     ...
     */
    public TextFieldCafe(String init, double x, double y, double width, IFuncParam1<Void, String> onTextArea, EnumVisibleType type) {
        super(init, 1);
        ...
        this.setBounds((int) x, (int) y, (int) width, this.getFontMetrics(this.getFont()).getHeight());
    ...
    public TextFieldCafe setDisplayType(int id, EnumDisplayType type) {
        if (id == 1)
            this.yType = type;
        else if (id == 2)
            this.widthType = type;
    ...
    public TextFieldCafe setOffset(int x) {
        this.offset = x;
        return this;
    }
}
